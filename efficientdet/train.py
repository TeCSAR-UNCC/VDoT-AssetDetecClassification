...
if load_model:
    load_checkpoint(torch.load("vdot_checkpoint_adam.pth"))'''
losses_val=[]
def save_checkpoint(state, filename="vdot_checkpoint_adam.pth"):
...
def train_epoch(
        epoch, model, loader, optimizer, args,  output_dir='./op_dir'):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                logging.info(
                    'Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                    'Loss: {loss.val:>9.6f} ({loss.avg:>6.9f})  '
        ...
        # end for
        loss_collective=OrderedDict([('loss',losses_m.avg)])
    all_losses.append(loss_collective['loss'])
...
def validate(model, loader, args, evaluator=None, log_suffix=''):
    ...
    losses_val.append(metrics['loss'])
    print(metrics)
    if evaluator is not None:
